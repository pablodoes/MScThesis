...
class KolSol(BaseNumpyKolSol):
    ...
    '''    
    def dynamics_control(self, u_hat: np.ndarray,beta) -> np.ndarray:
        ...
        # Canuto EQ [7.2.11]
        du_hat_dt = (f_hat + self.f) - kk_fk - (beta / self.re) * oe.contract('...u, ... -> ...u', u_hat, self.kk)

        return du_hat_dt
    '''    
    def dynamics_control_var(self, u_hat: np.ndarray, beta: float) -> np.ndarray:
        # 1) Anti-aliasing (igual que antes)
        aapt = np.array([[self.aap(u_hat[..., j], u_hat[..., i])
                        for j in range(self.ndim)]
                        for i in range(self.ndim)])
        # 2) f_hat con np.einsum
        f_hat = np.einsum('...t,ut...->...u', -self.nabla, aapt)

        # 3) k_dot_f sin dividir por cero
        tmp = f_hat + self.f
        numer = np.einsum('...u,...u->...', self.kt, tmp)  # numerador
        k_dot_f = np.zeros_like(numer)                     # lo inicializas a cero
        # sólo divides donde kk != 0
        np.divide(numer, self.kk, out=k_dot_f, where=(self.kk != 0))

        # 4) kk_fk
        kk_fk = self.kt * k_dot_f[..., None]

        # 5) du_hat_dt
        du_hat_dt = tmp - kk_fk - (beta / self.re) * (u_hat * self.kk[..., None])
        return du_hat_dt
    
    def dynamics_control_betaSin(self, u_hat: np.ndarray, beta: float) -> np.ndarray:
        # 1) Anti-aliasing (igual que antes)
        aapt = np.array([[self.aap(u_hat[..., j], u_hat[..., i])
                        for j in range(self.ndim)]
                        for i in range(self.ndim)])
        ...
        # 5) du_hat_dt
        du_hat_dt = tmp - kk_fk - (1.0 / self.re) * (u_hat * self.kk[..., None])
        return du_hat_dt
    
    def dynamics_control_betaSinVar2(self, u_hat: np.ndarray, beta: float) -> np.ndarray:
        # 1) Anti-aliasing (igual que antes)
        aapt = np.array([[self.aap(u_hat[..., j], u_hat[..., i])
                        for j in range(self.ndim)]
                        for i in range(self.ndim)])
...
def diffusion_energy_fourier(self, u_hat: np.ndarray) -> np.ndarray:
    """
    Compute total dissipation energy from the diffusion term in Fourier space.
    ...
    -------
    np.ndarray
        Scalar field representing local (Fourier-space) dissipation energy density.
    """

    # generate indices for einsum contraction
    g = iter(string.ascii_letters)
    idx = ''.join(next(g) for _ in range(self.ndim))

    # Compute energy: sum_i k^2 * |u_hat_i|^2
    diff_energy = oe.contract(f'...{idx} -> ...', self.kk * np.sum(np.abs(u_hat)**2, axis=-1))

    # Normalize as in dissip() to be comparable
    diff_energy /= (self.re * self.nk_grid ** (2 * self.ndim))

    return diff_energy.real